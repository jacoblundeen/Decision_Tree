...
class CART(object):
    ...
    def _grow_tree(self, features, target, criterion='gini'):
        ...
        for col in features.columns:
            ...
            for threshold in thresholds:
                target_l = target[(features[col] <= threshold).any()]
                impurity_l = self._calc_impurity(criterion, target_l)
                n_l = float(target_l) / self.n_samples

                target_r = target[(features[col] > threshold).any()]
                impurity_r = self._calc_impurity(criterion, target_r)
                n_r = float(target_r) / self.n_samples
    ...
    def _split_tree(self, features, target, criterion):
        ...
        target_l = target[(features[self.feature] <= self.threshold).any()]
        self.left = CART()
        self.left.depth = self.depth + 1
        self.left._grow_tree(features_l, target_l, criterion)

        features_r = features[(features[self.feature] > self.threshold).any(0)]
        target_r = target[(features[self.feature] > self.threshold).any()]
        self.right = CART()
